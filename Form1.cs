        ...
        private Region MakeRegion(int cx, int cy)
        {
            ...
            for(int xx = 0; xx <4; xx++)
            {
                for(int yy = 0; yy<4; yy++)
                {
                    if(BlackValue.bvals[bn,tn,xx,yy]!= 0)
                    {
                        Rectangle rect1 = new Rectangle((now.X+xx) * bwidth, (now.Y+yy) * bwidth, bwidth, bheight);
                        Rectangle rect2 = new Rectangle((now.X+xx + cx) * bwidth, (now.Y+yy + cy) * bwidth, bwidth, bheight);
        ...
        private Region MakeRegion()
        {
            ...
            for (int xx = 0; xx < 4; xx++)
            {
                for (int yy = 0; yy < 4; yy++)
                {
                    ...
                    if (BlackValue.bvals[bn, oldtn, xx, yy] != 0)
                    {
                        Rectangle rect1 = new Rectangle((now.X + xx) * bwidth, (now.Y + yy) * bwidth, bwidth, bheight);
                        Region rg1 = new Region(rect1);
                        region.Union(rg1);
